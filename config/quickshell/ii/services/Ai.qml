...
Singleton {
    ...
    property Component cursorApiStrategy: CursorApiStrategy {} // ShadowsRealm: Cursor provider
    ...
    property var tools: {
        ...
        "cursor": { // ShadowsRealm: Cursor uses the OpenAI tool schema
            "functions": [
                {
                    "type": "function",
                    "function": {
    ...
    property var models: Config.options.policies.ai === 2 ? {} : {
        ...
        "cursor-default": aiModelComponent.createObject(this, { // ShadowsRealm: Cursor provider
            "name": "Cursor",
            "icon": "cursor-symbolic",
            "description": Translation.tr("Online | Cursor API (OpenAI-compatible)\nRouted through your configured Cursor endpoint."),
            "homepage": "https://cursor.com",
            "endpoint": Config.options?.ai?.cursorEndpoint ?? "https://api.cursor.sh/v1/chat/completions",
            "model": "gpt-4o",
            "requires_key": true,
            "key_id": "cursor",
            "key_get_link": "https://cursor.com/settings",
            "key_get_description": Translation.tr("**Instructions**: Get a Cursor API key from your Cursor account settings, then set it here with `/key`.\n\nThe endpoint is configurable via `ai.cursorEndpoint` in the shell config."),
            "api_format": "cursor",
        }),
    }
    ...
    property var apiStrategies: {
        ...
        "cursor": cursorApiStrategy.createObject(this), // ShadowsRealm
    }
